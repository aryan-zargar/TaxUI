...
export default function AddServiceInvoice() {
    var [StorageOptions, setStorageOptions] = useState()
    var [CustomerOptions, setCustomerOptions] = useState()
    var [serviceInvoiceDetail, setserviceInvoiceDetail] = useState([{ discount2: 0, signedForDelete: false,rowIndex:0 }])
    ...
    function consolea(e) {
        e.preventDefault()
        data.serviceInvoiceDetails = serviceInvoiceDetail
        console.log(data)
    }
    ...
    function addNewserviceInvoiceDetail() {
        var newserviceInvoiceDetail = ({ discount2: 0, signedForDelete: false,rowIndex:serviceInvoiceDetail[serviceInvoiceDetail.length-1].rowIndex+1 })
        setserviceInvoiceDetail([...serviceInvoiceDetail,newserviceInvoiceDetail])
    }
    function removeserviceInvoiceDetail(index){
        let data = [...serviceInvoiceDetail];
        data.splice(index, 1)
        setserviceInvoiceDetail(data)
    }
    return (
            ...
            <LocalizationProvider dateAdapter={AdapterMomentJalaali} >
                <form className='grid grid-cols-4 p-12 container flex-wrap ' onSubmit={(e) => consolea(e)} dir='rtl' >
                    ...
                    <div className='col-span-4' >
                        {serviceInvoiceDetail.map((e, index) => {
                            return ( 
                                    ...
                                    <div className='grid grid-cols-4' >
                                        <div className='my-5' >
                                            ...
                                            <Autocomplete
                                                dir='rtl'
                                                color="neutral"
                                                onChange={(e, newVal) => serviceInvoiceDetail[index].productID = newVal.id}
                                                getOptionLabel={e => e.name}
                                                className='mx-3 mt-1'
                                                style={{ fontFamily: 'Rubik' }}
                                        ...
                                        <div className='my-5' >
                                            <label>تعداد</label>
                                            <Input color="neutral" onChange={(e) => serviceInvoiceDetail[index].serviceCount = Number(e.target.value)} type='number' className='mx-3 mt-1' style={{ fontFamily: 'Rubik' }} placeholder='تعداد' required variant="outlined" />
                                        </div>
                                        <div className='my-5' >
                                            <label>قیمت هر خدمت</label>
                                            <Input color="neutral" onChange={(e) => serviceInvoiceDetail[index].unitPrice = Number(e.target.value)} startDecorator={<p>تومان</p>} type='number' className='mx-3 mt-1' style={{ fontFamily: 'Rubik' }} placeholder='100000' required variant="outlined" />
                                        </div>
                                        <div className='my-5' >
                                            <label>درصد تخفیف</label>
                                            <Input color="neutral" onChange={(e) => serviceInvoiceDetail[index].discountPercent = Number(e.target.value)} startDecorator={<p>%</p>} type='number' className='mx-3 mt-1' style={{ fontFamily: 'Rubik' }} placeholder='10' required variant="outlined" />
                                        </div>
                                        <div className='my-5' >
                                            <label>درصد مالیات</label>
                                            <Input color="neutral" onChange={(e) => serviceInvoiceDetail[index].taxPercent = Number(e.target.value)} startDecorator={<p>%</p>} type='number' className='mx-3 mt-1' style={{ fontFamily: 'Rubik' }} placeholder='10' required variant="outlined" />
                                        </div>
                                        <div className='my-12'>

                                                <Button color='danger' onClick={()=>removeserviceInvoiceDetail(index)} > حذف </Button>
                                        </div>
                                    </div>
                        ...
                        <div className='mt-10' >
                            <Button onClick={() => {addNewserviceInvoiceDetail()}} >محصول جدید + </Button>
                        </div>
                    </div>
                    <div className=' mt-20' >
